...
void StoreMoney(){
	__asm{
		push eax;
		push ecx;
		mov eax, ds:[0x0086D8C4] ;
		mov eax, dword ptr[eax+0x2719B0];
		mov eax, dword ptr[eax+0x0];
		mov eax, dword ptr[eax+0x4];
		mov eax, dword ptr[eax+0x230];
		push eax;//All money
		push 0x3;//Dest (storage)
		push 0x2;//Source (inv)
		mov ecx,0x0092B1EC;
		mov eax,0x0064A6A0;
		call eax;
...
LRESULT CALLBACK NewWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if (uMsg == WM_HOOK_WRITE)
	{
		...
		case cmd_PShopping:
			...
			break;
		case cmd_SellingItem:
			SellingItem(lParam);
			break;
		case cmd_SellAndStore:
			SellingItem(lParam);
			StoreMoney();
			break;
		case cmd_OpenSupportMenu:
			OpenSupportMenu();
			break;
		case 80:
			strcpy(szData, "");
